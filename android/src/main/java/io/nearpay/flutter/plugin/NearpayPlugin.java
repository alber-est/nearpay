...
// Import new IC card reader classes
import io.nearpay.flutter.plugin.iccard.IcCardReader;
import io.nearpay.flutter.plugin.hardware.HardwareDetectionManager;
...
public class NearpayPlugin implements FlutterPlugin, MethodCallHandler {
    ...
    // New hardware components for IC card reader support
    private IcCardReader icCardReader;
    private HardwareDetectionManager hardwareManager;
    private HardwareDetectionManager.HardwareType currentHardwareType;
    ...
    public void onAttachedToEngine(@NonNull FlutterPluginBinding flutterPluginBinding) {
        ...
        // Initialize hardware detection and management
        hardwareManager = new HardwareDetectionManager(this.context);
        
        // Detect and initialize the best available hardware
        initializeHardware();
    }
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        ...
        if (call.method.equals("purchase")) {
          ...
          nearPay.close();
        } else if (call.method.equals("detectHardware")) {
            detectHardware(callUUID);
        } else if (call.method.equals("initializeHardware")) {
            String hardwareType = call.argument("hardwareType");
            String devicePath = call.argument("devicePath");
            initializeSpecificHardware(callUUID, hardwareType, devicePath);
        } else if (call.method.equals("readCard")) {
            readCard(callUUID);
        } else if (call.method.equals("startCardReading")) {
            startCardReading(callUUID);
        } else if (call.method.equals("stopCardReading")) {
            stopCardReading(callUUID);
        } else if (call.method.equals("getHardwareStatus")) {
            getHardwareStatus(callUUID);
        } else {
    ...
    /**
     * Initialize hardware detection and select best available option
     */
    private void initializeHardware() {
        try {
            ...
            HardwareDetectionManager.HardwareInfo preferredHardware = hardwareManager.getPreferredHardware();
            if (preferredHardware != null) {
                currentHardwareType = preferredHardware.type;
                
                if (preferredHardware.type == HardwareDetectionManager.HardwareType.IC_CARD_READER) {
                    setupIcCardReader(preferredHardware.devicePath);
                    Log.i("NearpayPlugin", "Initialized with IC Card Reader: " + preferredHardware.devicePath);
                } else {
                    Log.i("NearpayPlugin", "Using hardware type: " + preferredHardware.type.getValue());
                }
            } else {
                Log.w("NearpayPlugin", "No hardware detected, falling back to NearPay Cloud");
                currentHardwareType = HardwareDetectionManager.HardwareType.NEARPAY_CLOUD;
            }
        } catch (Exception e) {
            Log.e("NearpayPlugin", "Hardware initialization failed: " + e.getMessage());
            currentHardwareType = HardwareDetectionManager.HardwareType.NEARPAY_CLOUD;
        }
    }
    
    /**
     * Setup IC card reader
     */
    private void setupIcCardReader(String devicePath) {
        try {
            icCardReader = new IcCardReader(this.context);
            icCardReader.setCardListener(new IcCardReader.IcCardListener() {
                @Override
                public void onCardRead(Map<String, Object> cardData) {
                    // Convert card read to payment-like response for compatibility
                    Map<String, Object> response = new HashMap<>();
                    response.put("status", 200);
                    response.put("message", "Card read successfully");
                    response.put("card_data", cardData);
                    response.put("hardware_type", "ic_card_reader");
                    
                    Log.i("NearpayPlugin", "IC Card Read: " + cardData.get("cardId"));
                    
                    // Notify Flutter side about card detection
                    channel.invokeMethod("onCardDetected", response);
                }
                
                @Override
                public void onError(String errorMessage, int errorCode) {
                    Log.e("NearpayPlugin", "IC Card Reader Error: " + errorMessage);
                    ...
                    channel.invokeMethod("onCardError", errorResponse);
                }
                
                @Override
                public void onReaderConnected() {
                    Log.i("NearpayPlugin", "IC Card Reader Connected");
                    Map<String, Object> response = new HashMap<>();
                    response.put("status", 200);
                    response.put("message", "Card reader connected");
                    channel.invokeMethod("onReaderConnected", response);
                }
                
                @Override
                public void onReaderDisconnected() {
                    Log.w("NearpayPlugin", "IC Card Reader Disconnected");
                    Map<String, Object> response = new HashMap<>();
                    response.put("status", 400);
                    response.put("message", "Card reader disconnected");
                    channel.invokeMethod("onReaderDisconnected", response);
                }
            });
            
            boolean initialized = icCardReader.initialize(devicePath);
            if (!initialized) {
                icCardReader = null;
                throw new Exception("Failed to initialize IC card reader");
            }
            
        } catch (Exception e) {
            Log.e("NearpayPlugin", "IC Card Reader setup failed: " + e.getMessage());
            icCardReader = null;
        }
    }
     ...
     */
    private void detectHardware(String callUUID) {
        try {
            if (hardwareManager == null) {
                hardwareManager = new HardwareDetectionManager(this.context);
            }
            
            List<HardwareDetectionManager.HardwareInfo> hardwareList = hardwareManager.detectAllHardware();
            
            Map<String, Object> response = new HashMap<>();
            response.put("status", 200);
            response.put("message", "Hardware detection completed");
            response.put("current_hardware", currentHardwareType != null ? currentHardwareType.getValue() : "none");
            
            List<Map<String, Object>> hardwareMapList = new ArrayList<>();
            for (HardwareDetectionManager.HardwareInfo hardware : hardwareList) {
                hardwareMapList.add(hardware.toMap());
            }
            response.put("available_hardware", hardwareMapList);
            
            sendResponse(response, callUUID);
            
        } catch (Exception e) {
            Log.e("NearpayPlugin", "Hardware detection failed: " + e.getMessage());
            Map<String, Object> errorResponse = commonResponse(ErrorStatus.general_failure_code, 
                "Hardware detection failed: " + e.getMessage());
            sendResponse(errorResponse, callUUID);
        }
    }
     ...
     */
    private void initializeSpecificHardware(String callUUID, String hardwareType, String devicePath) {
        try {
            HardwareDetectionManager.HardwareType type = HardwareDetectionManager.HardwareType.valueOf(hardwareType.toUpperCase());
            
            if (type == HardwareDetectionManager.HardwareType.IC_CARD_READER) {
                setupIcCardReader(devicePath != null ? devicePath : "/dev/ttyUSB0");
                currentHardwareType = type;
                
                Map<String, Object> response = commonResponse(ErrorStatus.success_code, 
                    "IC Card Reader initialized successfully");
                response.put("hardware_type", type.getValue());
                response.put("device_path", devicePath);
                sendResponse(response, callUUID);
                
            } else if (type == HardwareDetectionManager.HardwareType.NEARPAY_CLOUD) {
                currentHardwareType = type;
                
                Map<String, Object> response = commonResponse(ErrorStatus.success_code, 
                    "NearPay Cloud mode activated");
                response.put("hardware_type", type.getValue());
                sendResponse(response, callUUID);
                
            } else {
                Map<String, Object> response = commonResponse(ErrorStatus.invalid_argument_code, 
                    "Unsupported hardware type: " + hardwareType);
                sendResponse(response, callUUID);
            }
            ...
            Log.e("NearpayPlugin", "Hardware initialization failed: " + e.getMessage());
            Map<String, Object> errorResponse = commonResponse(ErrorStatus.general_failure_code, 
                "Hardware initialization failed: " + e.getMessage());
            sendResponse(errorResponse, callUUID);
        }
    }
    
    /**
     * Read a card using current hardware
     */
    private void readCard(String callUUID) {
        if (currentHardwareType == HardwareDetectionManager.HardwareType.IC_CARD_READER) {
            if (icCardReader != null && icCardReader.isConnected()) {
                icCardReader.readSingleCard();
                
                Map<String, Object> response = commonResponse(ErrorStatus.success_code, 
                    "Card reading initiated");
                response.put("hardware_type", "ic_card_reader");
                sendResponse(response, callUUID);
                
            } else {
                Map<String, Object> response = commonResponse(ErrorStatus.invalid_status_code, 
                    "IC Card Reader not connected");
                sendResponse(response, callUUID);
            }
        } else {
            Map<String, Object> response = commonResponse(ErrorStatus.invalid_argument_code, 
                "Card reading not supported with current hardware type: " + currentHardwareType);
            sendResponse(response, callUUID);
        }
    }
     ...
     */
    private void startCardReading(String callUUID) {
        if (currentHardwareType == HardwareDetectionManager.HardwareType.IC_CARD_READER) {
            if (icCardReader != null && icCardReader.isConnected()) {
                icCardReader.startReading();
                
                Map<String, Object> response = commonResponse(ErrorStatus.success_code, 
                    "Card reading started");
                response.put("hardware_type", "ic_card_reader");
                sendResponse(response, callUUID);
            ...
            Map<String, Object> response = commonResponse(ErrorStatus.invalid_argument_code, 
                "Continuous card reading not supported with current hardware");
            sendResponse(response, callUUID);
        }
    }
     ...
     * Stop card reading
     */
    private void stopCardReading(String callUUID) {
        if (icCardReader != null) {
            icCardReader.disconnect();
            
            Map<String, Object> response = commonResponse(ErrorStatus.success_code, 
                "Card reading stopped");
            sendResponse(response, callUUID);
        } else {
            Map<String, Object> response = commonResponse(ErrorStatus.success_code, 
                "No active card reading to stop");
            sendResponse(response, callUUID);
        }
    }
     ...
     */
    private void getHardwareStatus(String callUUID) {
        try {
            Map<String, Object> response = new HashMap<>();
            response.put("status", 200);
            response.put("message", "Hardware status retrieved");
            response.put("current_hardware", currentHardwareType != null ? currentHardwareType.getValue() : "none");
            
            if (icCardReader != null) {
                Map<String, Object> icReaderStatus = icCardReader.getDeviceInfo();
                response.put("ic_reader_status", icReaderStatus);
            }
            
            if (nearPay != null) {
                response.put("nearpay_initialized", true);
            } else {
                response.put("nearpay_initialized", false);
            }
            
            sendResponse(response, callUUID);
            
        } catch (Exception e) {
            Log.e("NearpayPlugin", "Status check failed: " + e.getMessage());
            Map<String, Object> errorResponse = commonResponse(ErrorStatus.general_failure_code, 
